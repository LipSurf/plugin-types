...
declare type SingleTest = (t: ExecutionContext<ICommandTestContext>, say: (s?: string, resultIndex?: number) => Promise<void>, client: WebdriverIOAsync.BrowserObject) => Promise<void>|void;
...
declare interface IPluginUtil {
    ...
    fuzzyHighScore: (query: string, sources: string[], minScore?: number) => Promise<[number, number]>;
    ...
    disambiguate(els: FrameEleWOffsets[]): Promise<[FrameEleWOffsets, Promise<void>]>;
    disambiguate(els: HTMLElement[]): Promise<[HTMLElement, Promise<void>]>;
    disambiguate(els: HTMLElement[]|FrameEleWOffsets[]): Promise<[HTMLElement|FrameEleWOffsets, Promise<void>]>;
